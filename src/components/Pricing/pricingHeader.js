...
const PricingPlan = () => {
  ...
  const plans = [
    {
      ...
      yearlyPrice : 99.99,
      slider: false,
      features: ['LinkedIn post', 'Twitter post', 'Instagram post'],
      extraFeature: false,
    ...
    },
    {
      ...
      yearlyPrice: 109.99,
      slider: true,
      features: ['LinkedIn posts', 'Twitter posts', 'Instagram posts'],
      extraFeature: true,
  ...
  <div className="flex justify-center w-full px-4 ">
    {plans.map((plan, index) => (
      <PricingCard key={plan.id} plan={plan} isYearlyBilling={isYearlyBilling} index={index} />
    ))}
    </div>
  </div>
  );
